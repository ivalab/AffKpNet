...
M_CL = np.array([[-0.10143271, -0.98296922,  0.15324134, -0.11591623],
                 [-0.74678104, -0.026539,   -0.66454026,  0.10929024],
                 [ 0.6572895,  -0.18184385, -0.73137086,  0.6467287 ],
                 [ 0.,          0.,          0.,          1.        ]]
                )
...
def test(args):
    ...
    while True:
        ...
        for instance_name in instance_list:
            ...
            cv2.imshow('ori image', img_ori)
            cv2.namedWindow('mask', cv2.WINDOW_AUTOSIZE)
            cv2.imshow('mask', mask)
            cv2.waitKey(0)

            # cv2.imwrite(os.path.join(Img_Root, object_name, 'mask.png'), mask)
