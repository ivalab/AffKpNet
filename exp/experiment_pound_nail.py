...
def inference(args, pipeline, align, depth_scale, pub_res):
    ...
    while not rospy.is_shutdown():
        ...
        # detection of grasp affordace
        g_p_c_3d = None
        g_angle = 0.0
        # detection of pound affordance
        p_p_c_3d = None
        p_p_1_3d = None
        p_p_2_3d = None
        ...
        for kp in kps_final:
            if kp[11] == 0:
                ...
                # project grasp center to 3d
                p_c = [int(kp[8]), int(kp[9])]
                p_p_c_3d = project(p_c, depth_ori, M_CL, M_BL, cameraMatrix)

                # compute orientation
                p_1 = [int(kp[0]), int(kp[1])]
                p_2 = [int(kp[2]), int(kp[3])]
                p_p_1_3d = project(p_1, depth_ori, M_CL, M_BL, cameraMatrix)
                p_p_2_3d = project(p_2, depth_ori, M_CL, M_BL, cameraMatrix)
