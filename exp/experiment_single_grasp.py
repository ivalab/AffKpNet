...
M_CL = np.array([[-0.07134498, -0.99639369,  0.0459293,  -0.13825178],
...
def project(pixel, depth_image, M_CL, M_BL, cameraMatrix):
    ...
    while depth == 0:
        moving_pixel = [moving_pixel[0]+1, moving_pixel[1]]
...
def test(args):
    ...
    while True:
        ...
        for instance_name in instance_list:
            ...
            # convert 5 classes into by voting for the majority
            kps = []
            for pred_idx in range(pred_kps.shape[0]):
                ...
                if pred_kp[10] > 0.1:
                    ...
                    # Case 2: five keypoints should have the same label
                    ...
                    # kps.append(kp_new)
                # kps[pred_idx, :11] = pred_kp[:11]
                # kps[pred_idx, 11] = cls
                # kps[pred_idx, 12:17] = pred_kp[16:21]
            kps = np.array(kps)
                ...
                for idx1 in range(5):
                    for idx11 in range(idx1 + 1, 5):
                        if (idx1 == 3 and idx11 == 5) or (idx1 == 4 and idx11 == 5):
                            continue
                        if np.sqrt(
                            np.sum((kp[idx1 * 2:(idx1 + 1) * 2] - kp[idx11 * 2:(idx11 + 1) * 2]) ** 2)) < 4 * np.sqrt(
                            2):
                            flag = False
                if flag:
                    kps_v1.append(kp)
            kps_v1 = np.array(kps_v1)

            if kps_v1.size == 0:
                continue

            # # remove highly overlapped grouping results
            kps_v2 = np.expand_dims(kps_v1[0, :], axis=0)  # always select the first one
            for kp in kps_v1:
                flag = True
                for idx111 in range(kps_v2.shape[0]):
                    if np.sqrt(np.sum((kps_v2[idx111, :10] - np.expand_dims(kp[:10], axis=0)) ** 2)) < 40:
                        flag = False

                if flag:
                    kps_v2 = np.concatenate((kps_v2, np.expand_dims(kp, axis=0)), axis=0)

            # kps_v2 = geometric_contrain_filter(kps_v2)

            # single output per aff
            kps_v3 = single_aff_filter(kps_v2)
